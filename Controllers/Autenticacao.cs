    ...
    public class Autenticacao
    {
        ...
        public static bool verificaLoginSenha(string login, string senha, Controller controller)
        {
            using(BibliotecaContext bc = new BibliotecaContext())
            {
                ...
                List<Usuario>ListaUsuarioEncotrado = UsuarioEncontrado.ToList();

                if(ListaUsuarioEncotrado.Count==0)
                {
                    return false;
                }else{
                    controller.HttpContext.Session.SetString("login", ListaUsuarioEncotrado[0].Login);
                    controller.HttpContext.Session.SetString("Nome", ListaUsuarioEncotrado[0].Nome);
                    controller.HttpContext.Session.SetInt32("tipo", ListaUsuarioEncotrado[0].Tipo);
                    return true;
                }
            }
        }
